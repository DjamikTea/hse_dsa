...
async def revoke(phone: str, request: Request, db=Depends(get_db)):
    ...
    if requestx is not None:
        if requestx['time'].replace(tzinfo=timezone.utc) > datetime.now(timezone.utc) - timedelta(minutes=5):
    ...
    cursor.execute("INSERT INTO revoke_requests (phone_number, pubkey, ip, time, tries, verif_code, request_id)"
        "VALUES (%s, %s, %s, NOW(), 0, %s, %s)", (phone, user['pubkey'], ip, verif_code, resp['result']['request_id']))
    conn.commit()
...
async def revoke_verify(phone: str, code: str, request: Request, db=Depends(get_db)):
    ...
    if requestx['verif_code'] != code:
        cursor.execute("UPDATE revoke_requests SET tries = %s WHERE phone_number = %s", (requestx['tries'] + 1, phone))
        conn.commit()
        raise HTTPException(status_code=400, detail="Invalid verification code")

    cursor.execute("INSERT INTO revoked_keys (phone_number, pubkey, time, ip) VALUES (%s, %s, NOW(), %s)", (phone, requestx['pubkey'], requestx['ip']))
