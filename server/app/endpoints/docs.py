...
@router.put("/upload")
async def new_document(file: UploadFile = File(...), sha256: str = Header(None),  authorization: Optional[str] = Header(None), db=Depends(get_db)):
    """
    ...
    try:
        cursor.execute("INSERT INTO documents (timeuuid, path, filename, user_id, created, sha256) "
                       "VALUES (%s, %s, %s, %s, NOW(), %s)", (timeuuid, file_location, file_name, user['id'], sha256))
        conn.commit()
...
async def sign_document(timeuuid: str, signature: str = Header(None), authorization: Optional[str] = Header(None), db=Depends(get_db)):
    """
    ...
    if not check_document(signaturex, document['sha256'], user_pubkey, timeuuid, root_pubkey['pubkey']):
        raise HTTPException(status_code=400, detail="Invalid signature")

    cursor.execute("UPDATE documents SET sign_verified = 1, sign = %s WHERE timeuuid = %s", (signature, timeuuid,))
    conn.commit()
    return {"status": "ok"}

@router.get("/download/{timeuuid}")
async def download_document(timeuuid: str, authorization: Optional[str] = Header(None), db=Depends(get_db)):
    """
    Скачивает документ
    :param timeuuid:
    :param authorization:
...
async def send_document(timeuuid: str, phone: str = Header(None), authorization: Optional[str] = Header(None), db=Depends(get_db)):
    """
    ...
    cursor.execute("UPDATE documents SET can_access = %s WHERE timeuuid = %s", (recivers, timeuuid,))
    cursor.execute("INSERT INTO new_docs_available (user_id, timeuuid, filename, created, sign, sha256) "
                "VALUES (%s, %s, %s, NOW(), %s, %s)",
                   (reciver['id'], timeuuid, document['filename'], document['sign'], document['sha256']))
    conn.commit()
    return {"message": f"Document sent to {phone}"}

@router.get("/available")
async def available_documents(authorization: Optional[str] = Header(None), db=Depends(get_db)):
    """
...
async def accept_document(timeuuid: str, authorization: Optional[str] = Header(None), db=Depends(get_db)):
    """
    ...
    cursor.execute("SELECT * FROM new_docs_available WHERE timeuuid = %s AND user_id = %s", (timeuuid, user['id']))
    document = cursor.fetchone()
    if not document:
        raise HTTPException(status_code=404, detail="Not found")

    cursor.execute("DELETE FROM new_docs_available WHERE timeuuid = %s AND user_id = %s", (timeuuid, user['id']))
    conn.commit()
