...
def generate_csr(private_key: str, public_key: str, country: str, organization: str, phone_number: str, ip: str,
                 fio: str) -> dict:
    """
...
def check_csr_client(csr: dict, phone_number: str = None, ip: str = None) -> bool:
    ...
    if ip:
        if csr['client']['ip'] != ip:
            return False
    return crypto.check(csr['client_sign'], str(csrx).encode(), csr['client']['public_key'])


def sign_csr(csr: dict, root_private_key: str, root_ca: dict, phone_number: str = None, ip: str = None) -> dict:
    """
...
def check_csr_root(csr: dict, server_domain: str = None, server_pubkey: str = None) -> bool:
    ...
    return crypto.check(csr['root_sign'], str(csrx).encode(), csr['root']['root_ca']['public_key'])
